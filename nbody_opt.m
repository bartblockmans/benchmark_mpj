function nbody_opt()
% N-Body Galaxy Simulation in MATLAB (OPTIMIZED VERSION)
% ====================================================
%
...
% - Preallocated acceleration arrays to avoid per-step memory allocation
% - Kernel warm-up to exclude compilation time from performance measurements
% - Precomputed constants and single precision throughout for optimal performance
% - MATLAB vectorized integration (O(N)) + optimized scalar loops (O(N²))
%
% Key Features:
...
% - All optimizations are language-specific (no external libraries beyond standard MATLAB)
...
% - Run with: matlab -singleCompThread -nodesktop -nosplash for best performance
%
...
% =========================
% To ensure fair single-threaded performance comparison:
% 1. Launch MATLAB with: matlab -singleCompThread -nodesktop -nosplash
% 2. Or set environment variable: set OMP_NUM_THREADS=1 (Windows) / export OMP_NUM_THREADS=1 (Linux/Mac)
% 3. The script will automatically use optimal settings for maximum performance
%
...
%% =============================================================================
% MATLAB-SPECIFIC OPTIMIZATION SETTINGS FOR BENCHMARKING
% =============================================================================
% Ensure single-threaded execution for fair comparison
if ~isempty(which('maxNumCompThreads'))
    maxNumCompThreads(1);  % Force single-threaded execution
end

% Disable parallel pool if it exists
...
STOREFINAL  = false;              % Save initial & final PNGs to ./matlab/ directory
...
tEnd        = single(1.0);        % Total simulation time
...
function [x, y, z, u, v, w, m, G_actual, softening_actual] = import_initial_conditions(filename)
    ...
    %
    % Note: This function requires MATLAB R2016b+ for jsondecode support
    %
    % Author: Bart Blockmans
    % Date: August 2025
...
%% =============================================================================
% INITIALIZATION AND KERNEL WARM-UP (OPTIMIZED)
% =============================================================================
%
% This section performs critical optimizations:
% - Preallocates acceleration arrays to avoid per-step allocations
% - Kernel warm-up to exclude JIT compilation time from timing
% - Initial force computation using optimized kernels
% - Snapshot saving and simulation setup

% Preallocate acceleration arrays to avoid per-step allocations
ax = zeros(N_actual, 1, 'single');
ay = zeros(N_actual, 1, 'single');
az = zeros(N_actual, 1, 'single');
...
fprintf('Warming up JIT kernels...\n');
fprintf('Using full array size N=%d for proper JIT compilation...\n', N_actual);
x_warm = x;
y_warm = y;
z_warm = z;
u_warm = u;
...
w_warm = w;
m_warm = m;
ax_warm = zeros(N_actual, 1, 'single');
ay_warm = zeros(N_actual, 1, 'single');
az_warm = zeros(N_actual, 1, 'single');

% Warm up the force computation kernels (all possible code paths)
if USE_SYM
    if USE_BLOCKED
        [ax_warm, ay_warm, az_warm] = compute_acc_sym_blocked_inplace(ax_warm, ay_warm, az_warm, x_warm, y_warm, z_warm, m_warm, Gconst, softening, BLOCK_SIZE);
    else
        [ax_warm, ay_warm, az_warm] = compute_acc_sym_inplace(ax_warm, ay_warm, az_warm, x_warm, y_warm, z_warm, m_warm, Gconst, softening);
    end
else
    [ax_warm, ay_warm, az_warm] = compute_acc_full_inplace(ax_warm, ay_warm, az_warm, x_warm, y_warm, z_warm, m_warm, Gconst, softening);
end

fprintf('Kernel warm-up complete! All functions compiled with correct signatures.\n');

% Compute initial accelerations using optimized kernels
fprintf('Computing initial forces...\n');
if USE_SYM
    if USE_BLOCKED
        [ax, ay, az] = compute_acc_sym_blocked_inplace(ax, ay, az, x, y, z, m, Gconst, softening, BLOCK_SIZE);
    else
        [ax, ay, az] = compute_acc_sym_inplace(ax, ay, az, x, y, z, m, Gconst, softening);
    end
else
    [ax, ay, az] = compute_acc_full_inplace(ax, ay, az, x, y, z, m, Gconst, softening);
end
...
if STOREFINAL
    ...
    if VISUALIZE
        ...
        PE_init = [potential_energy(x,y,z,m,Gconst,softening)];
...
if VISUALIZE
    ...
    trail_x  = zeros(TRAIL, N, 'single');    % X-positions for trail history
    trail_y  = zeros(TRAIL, N, 'single');    % Y-positions for trail history
    ...
    PEsave(1) = potential_energy(x,y,z,m,Gconst,softening);
    KPsave(1) = KEsave(1) + PEsave(1);
    times(1)  = 0.0;
end

%% =============================================================================
% MAIN TIME INTEGRATION LOOP (OPTIMIZED)
% =============================================================================
%
% This section uses truly optimized approach for maximum performance:
% - MATLAB vectorized integration for O(N) operations
% - In-place force computation for O(N²) operations  
% - Preallocated acceleration arrays to avoid memory allocation
% - Real-time visualization updates (when enabled)

for it = 1:Nt
    % LEAPFROG INTEGRATION SCHEME (using efficient MATLAB vectorized operations)
    %
    ...
    if VISUALIZE
        ...
        PEsave(it+1) = potential_energy(x,y,z,m,Gconst,softening);
        KPsave(it+1) = KEsave(it+1) + PEsave(it+1);
        times(it+1)  = double(t);

        % Update particle trails (ring buffer) - optimized to avoid mod per step
        trail_x(trail_idx,:) = x;
        trail_y(trail_idx,:) = y;
        trail_idx = trail_idx + 1;
        if trail_idx > TRAIL, trail_idx = 1; end  % Conditional wrap instead of mod (cheaper than mod)
...
function [ax, ay, az] = compute_acc_sym_inplace(ax, ay, az, x, y, z, m, G, eps)
% COMPUTE_ACC_SYM_INPLACE - Compute gravitational acceleration in-place (symmetric version)
%
...
%
% Inputs:
%   ax, ay, az - Pre-allocated acceleration component vectors (modified in-place)
...
%   - Uses in-place accumulation to avoid array allocations

    N = numel(x);
    ax(:) = single(0); ay(:) = single(0); az(:) = single(0);  % Reset acceleration arrays
...
function [ax, ay, az] = compute_acc_full_inplace(ax, ay, az, x, y, z, m, G, eps)
% COMPUTE_ACC_FULL_INPLACE - Compute gravitational acceleration in-place (full version)
%
...
%
% Algorithm:
%   - Computes all N² forces explicitly
%   - Slower but more straightforward implementation
%   - Uses in-place accumulation to avoid array allocations

    N = numel(x);
    ax(:) = 0; ay(:) = 0; az(:) = 0;  % Reset acceleration arrays
    eps2 = eps * eps;  % Softening parameter squared
...
function [ax, ay, az] = compute_acc_sym_blocked_inplace(ax, ay, az, x, y, z, m, G, eps, B)
% COMPUTE_ACC_SYM_BLOCKED_INPLACE - Compute gravitational acceleration with blocking for cache locality
%
...
%   - Maintains exact momentum conservation
%   - Uses in-place accumulation to avoid array allocations
    ...
    for ib = 1:B:N-1
        ...
        for jb = ib+1:B:N
            ...
            % Standard scalar loops inside the small tile
            for i = ib:iend
                xi = x(i); yi = y(i); zi = z(i); mi = m(i);
                j0 = max(i+1, jb);  % Honor i<j inside this tile
...
function [x_new, y_new, z_new, u_new, v_new, w_new, ax_new, ay_new, az_new] = step_inplace(x, y, z, u, v, w, m, ax, ay, az, dt, G, eps, use_symmetry, use_blocked, block_size)
% STEP_INPLACE - Fused leapfrog integration step to reduce overhead
%
...
%   ax, ay, az          - Pre-allocated acceleration arrays
%   dt                  - Time step
%   G                   - Gravitational constant
...
%
% Outputs:
%   x_new, y_new, z_new, u_new, v_new, w_new - Updated particle positions and velocities
%   ax_new, ay_new, az_new - Updated acceleration arrays

    % Precompute constants for efficiency
    halfdt = single(0.5) * dt;
    ...
    % Step 3: Update accelerations (forces) - compute new accelerations
    if use_symmetry
        if use_blocked
            [ax_new, ay_new, az_new] = compute_acc_sym_blocked_inplace(ax, ay, az, x_new, y_new, z_new, m, G, eps, block_size);
        else
            [ax_new, ay_new, az_new] = compute_acc_sym_inplace(ax, ay, az, x_new, y_new, z_new, m, G, eps);
        end
    else
        [ax_new, ay_new, az_new] = compute_acc_full_inplace(ax, ay, az, x_new, y_new, z_new, m, G, eps);
    end

    % Step 4: Complete the kick (update velocities by remaining half time step)
    u_new = u_new + halfdt .* ax_new;
    v_new = v_new + halfdt .* ay_new;
    w_new = w_new + halfdt .* az_new;
end
